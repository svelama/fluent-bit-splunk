...
echo ""
echo "2️⃣  Deploying Mock Splunk Server..."
kubectl apply -f "${PROJECT_ROOT}/manifests/base/07-mock-splunk.yaml"
echo "   Waiting for Mock Splunk to be ready..."
kubectl wait --for=condition=Ready pod -l app=mock-splunk -n splunk-mock --timeout=120s
...
echo ""
echo "   # Watch Mock Splunk (see received logs):"
echo "   kubectl logs -f -n splunk-mock -l app=mock-splunk"
echo ""
...
echo "  ✓ Logs from team-alpha and team-beta should appear in Mock Splunk"
echo "  ✗ Logs from team-gamma should NOT appear (no namespace label)"
echo ""
